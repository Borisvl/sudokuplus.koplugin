...
describe("core.solver", function()
    ...
    it("handles solving an already full board (0 empty cells)", function()
        local s = solver.new(board.from_string(UNIQUE_SOLUTION))
        local solutions = s:solve_all()
        assert.are.equal(1, #solutions)
        assert.are.equal(UNIQUE_SOLUTION, board.to_string(solutions[1].board))
        assert.are.equal(1, s:count_solutions())
    end)

    it("handles solving a board with 1 empty cell remaining", function()
        local b = board.from_string(UNIQUE_SOLUTION)
        board.set(b, 8, 8, 0)
        local s = solver.new(b)
        local solutions = s:solve_all()
        assert.are.equal(1, #solutions)
    ...
    it("handles solving a completely empty board (81 empty cells)", function()
        local empty_puzzle = string.rep("0", 81)
        local s = solver.new(board.from_string(empty_puzzle), { rng = prng.new(42) })
        local solution = s:solve_any()
        assert.is_not_nil(solution)
        assert.are.equal(81, board.count_clues(solution.board))
