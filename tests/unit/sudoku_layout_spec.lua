...
describe("sudoku layout", function()
    ...
    it("merges horizontally adjacent cells in a row into one strip", function()
        local l = layout.compute(758, 1024, scaler(GLO))
        local a = layout.cell_rect(l, 3, 4)
        local b = layout.cell_rect(l, 3, 5)
        local regions = layout.cells_regions(l, { [3 * 9 + 4] = true, [3 * 9 + 5] = true })
        assert.are.equal(1, #regions)
        assert.are.equal(a.x, regions[1].x)
        assert.are.equal(a.y, regions[1].y)
        assert.are.equal(b.x + b.w - a.x, regions[1].w, "strip spans the border between the cells")
        assert.are.equal(a.h, regions[1].h)
    end)

    it("keeps non-adjacent cells of a row as separate regions, ordered by row", function()
        local l = layout.compute(1072, 1448, scaler(CLARA))
