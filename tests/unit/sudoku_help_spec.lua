...
describe("sudoku help module", function()
    ...
    it("defines the two main help topics", function()
        local topics = help.topics()
        assert.is_table(topics)
        assert.are.equal(2, #topics)
    ...
    it("returns formatted text for the features topic", function()
        ...
        assert.is_not_nil(text:find("- **Step 1 (Strategy Name):**", 1, true))
        assert.is_not_nil(text:find("Naked Pairs and Locked Candidates", 1, true))
        assert.is_not_nil(text:find("Hidden Pairs, Naked Triples", 1, true))
        assert.is_not_nil(text:find("X-Wing, Skyscraper", 1, true))
