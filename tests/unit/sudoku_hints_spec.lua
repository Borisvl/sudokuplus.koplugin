...
describe("core.hints", function()
    ...
    it("reports illegal or empty note masks as note errors", function()
        ...
        local empty = state_for(NAKED_SINGLE_PUZZLE)
        candidates.set(empty.notes, 8, 8, 0)
        local empty_result = assert(hints.next(empty))
        assert.are.equal("note_error", empty_result.status)
        assert.are.equal("empty_candidate_mask", empty_result.errors[1].reason)
    end)
