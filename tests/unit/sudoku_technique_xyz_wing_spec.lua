...
-- HoDoKu XYZ-Wing example: https://hodoku.sourceforge.net/en/show_example.php?file=z101&tech=XYZ-Wing
local HODOKU = "069000000000021000000800400001530080007600050000000100000000003902080010000340205"
local techniques = bit.bor(flags.EASY, flags.XYZ_WING)
...
local function xyz_wing_steps(path)
    local steps = {}
    for _, step in ipairs(path.steps) do
        if step.type == "elim" and step.flags == flags.XYZ_WING then
            steps[#steps + 1] = step
        end
    end
    return steps
...
describe("core.techniques.xyz_wing", function()
    it("produces xyz-wing eliminations", function()
        local s = solver.new(board.from_string(HODOKU), { techniques = techniques })
        local path = solve_path.new()
        assert.is_true(s:propagate(path))
        local steps = xyz_wing_steps(path)
    ...
    it("records pattern metadata with pivot and pincers", function()
        ...
        local path = solve_path.new()
        s:propagate(path)
        for _, step in ipairs(xyz_wing_steps(path)) do
            local pattern = step.pattern
            assert.are.equal("xyz_wing", pattern.kind)
    ...
    it("eliminates only the z value from cells seeing all three cells", function()
        local s = solver.new(board.from_string(HODOKU), { techniques = techniques })
        local path = solve_path.new()
        s:propagate(path)
        for _, step in ipairs(xyz_wing_steps(path)) do
            ...
            local pivot = pattern.pivot
            local p1, p2 = pattern.pincers[1], pattern.pincers[2]
            assert.is_true(units.sees(step.row, step.col, pivot[1], pivot[2]))
